# Share kit — Certuvo · The CMA, Part 1 and Part 2

Platform copy for the film. Every line obeys the film's rules: CMA® with the
mark on first use, Certuvo described as an independent preparation provider,
no pass guarantee, no fee amounts, no pass-rate claims, and every fact
traceable to `claims-register.md`. Use as written or cut down — do not add to
it.

---

## Website (course page / landing page)

**Headline:** The CMA, explained in four minutes.

**Standfirst:** Two exams. Four hours each. Twelve domains. Here is exactly
what is in Part 1 and Part 2, how the exam is scored, what you need to be
certified — and how Certuvo gets you ready for it.

**Below the player:**
- Part 1 — Financial Planning, Performance and Analytics: six domains, 15–20 % each
- Part 2 — Strategic Financial Management: six domains, Decision Analysis the largest at 25 %
- 100 multiple-choice questions (75 %) then two case-based questions, from the September 2026 window
- 360 / 500 to pass · three testing windows a year
- IMA membership, a bachelor's degree or approved certification, two years' experience (completable up to seven years after passing)

**Footer line (required):** CMA® is a registered trademark of the Institute of
Management Accountants (IMA). Certuvo is an independent preparation provider and
is not affiliated with, sponsored by or endorsed by IMA. Confirm current exam
details at imanet.org. Preparation does not guarantee a pass.

---

## YouTube

**Title (≤ 100):** The CMA Exam Explained: Part 1 & Part 2, Format, Scoring and Requirements (2026)

**Description:**

> Everything you need to know about the Certified Management Accountant (CMA®) exam in four minutes — the twelve domains and their weights across Part 1 and Part 2, how each four-hour exam is scored, the case-based questions that replace the essays from the September 2026 window, the 50 % gate, the 360/500 pass mark, the three testing windows, and what IMA requires for certification.
>
> Chapters
> 0:00 Two exams, one credential
> 0:09 Welcome
> 0:30 What the CMA is
> 0:55 Part 1 — Financial Planning, Performance and Analytics
> 1:26 Part 2 — Strategic Financial Management
> 1:55 The exam: format, scoring, windows
> 2:33 Requirements
> 2:53 Why it matters
> 3:16 How Certuvo prepares you
> 3:46 Start
>
> Prepare with Certuvo → https://certuvo.com
>
> CMA® is a registered trademark of the Institute of Management Accountants (IMA). Certuvo is an independent preparation provider and is not affiliated with, sponsored by or endorsed by IMA. Exam structure and requirements as published by IMA for 2026; confirm current details at imanet.org. Preparation does not guarantee a pass.

**Tags:** CMA exam, CMA Part 1, CMA Part 2, Certified Management Accountant, IMA CMA, CMA exam format 2026, CMA case-based questions, management accounting, Certuvo

Upload `certuvo-cma-3840x2160-MASTER-clean.mp4` with `certuvo-cma.srt` as the
caption track (YouTube renders its own, so the clean master keeps the picture
uncluttered); or the captioned master if captions must be burned in.
Thumbnail: `certuvo-cma-thumbnail-1280x720.png`.

---

## LinkedIn (company page)

> **The CMA, in four minutes.**
>
> Two exams. Four hours each. Twelve domains. If you are weighing up the Certified Management Accountant (CMA®), this is the structure — exactly as it stands for 2026:
>
> ▸ Part 1 — Financial Planning, Performance and Analytics (six domains)
> ▸ Part 2 — Strategic Financial Management (six domains; Decision Analysis is the largest at 25 %)
> ▸ 100 multiple-choice questions, then two case-based questions from the September 2026 window
> ▸ 360 / 500 to pass · three testing windows a year
>
> And what IMA requires to be certified — including the seven-year window that lets you sit the exam first and complete the experience later.
>
> Certuvo prepares you for exactly this exam: every domain in the exam's own weightings, scenario-based question banks, full-length timed mocks, and readiness tracking that tells you when you're ready — not just how much you've read.
>
> Watch, then start → https://certuvo.com
>
> CMA® is a registered trademark of IMA. Certuvo is an independent preparation provider, not affiliated with or endorsed by IMA. Preparation does not guarantee a pass.
>
> #CMA #ManagementAccounting #CMAExam #FPandA #Controller #FinanceCareers #Certuvo

Use `certuvo-cma-1080x1080-captions.mp4` (feed) or the 16:9 captioned HD cut.

---

## Instagram / TikTok / Shorts (9:16 cut)

**Caption:**

> The CMA® exam, explained. Part 1, Part 2, the new case-based questions, the pass mark, the requirements — four minutes, no fluff. Link in bio → certuvo.com
>
> Certuvo is an independent CMA preparation provider, not affiliated with IMA. Preparation does not guarantee a pass.
>
> #CMA #CMAExam #ManagementAccounting #AccountingCareers #FinanceStudent #Certuvo

Use `certuvo-cma-1080x1920-captions.mp4`. For a Short/Reel under 60 s, cut
0:55–1:55 (Part 1 and Part 2 back to back) — it stands alone and ends on the
"boardroom" line.

---

## Email (to enquiries / leads)

**Subject:** The CMA exam in four minutes — Part 1, Part 2 and the 2026 format

> Hi {first name},
>
> You asked about the CMA. Rather than a brochure, here is a four-minute film that lays out the whole exam as it stands for 2026: the twelve domains and their weights across Part 1 and Part 2, how each four-hour exam is scored, the case-based questions that replace the essays from the September window, the pass mark, the testing windows, and what IMA requires to be certified.
>
> Watch it here: {link}
>
> When you are ready, Certuvo prepares you for exactly this exam — every domain in the exam's own weightings, scenario-based question banks, full-length timed mocks, and readiness tracking. Pick your part, choose a testing window, and start at certuvo.com.
>
> Kind regards,
> {name}, Certuvo
>
> CMA® is a registered trademark of the Institute of Management Accountants (IMA). Certuvo is an independent preparation provider and is not affiliated with, sponsored by or endorsed by IMA. Confirm current exam details at imanet.org. Preparation does not guarantee a pass.
