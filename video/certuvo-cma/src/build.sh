#!/usr/bin/env bash
# Build the Certuvo CMA film ("The CMA, Part 1 and Part 2").
#
...
# and writes the cut list that scene.html and vo.py both read.
set -euo pipefail
cd "$(dirname "$0")"
...
MUSIC="${MUSIC-../music/certuvo-bed.mp3}"
if [[ -n "$MUSIC" ]]; then
  ...
  echo "MUSIC= (no bed) is not supported for this film" >&2; exit 1
fi
...
POSTER_F=$((FPS*36)); THUMB_F=$((FPS*66))
