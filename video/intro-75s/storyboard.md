...
Source of truth: `SHOTS` and `CUES` in `src/scene.html`, which are themselves
derived by `src/sync.py` from where the recorded voice actually falls. Keep this
file in step.
