...
| **ElevenLabs** | Surveyed voices, then generated **124 narration segments** — 15 for the 75-second film, 4 for the 15-second launch film, 16 for the 4:16 explainer, a 4-voice A/B, and 57 for the Honorary Fellow film (10 scenes, 6 tightened retakes, 2 benefits retakes, 9 for the pitch cut, a 3-voice authority test, 13 for the authority cut on Jim Executive, 13 on Holden after PCI chose it, 13 more on Holden in a warmer, quicker direction, 13 more with phrase-level emotional direction, and 13 on Nassim — Corporate Narration when PCI named that voice, plus one Institute-scene retake on Nassim after PCI removed the accreditation line), plus 53 Scribe transcriptions verifying the takes against the script, and one 250-second ElevenLabs Music v2 instrumental bed | Three films on **"Jim Executive — Authoritative, British and Warm"** (`tXxkePQsw0G69D8VeDzp`). The Honorary Fellow film on **"Nassim — Corporate Narration"** (`repzAAjoKlgcT2oOAIWt`), the voice PCI named for the final cut, after earlier cuts on Holden Pro Voice (the brief's voice) and a three-voice test. The 75s and 15s films are on `eleven_multilingual_v2`; the explainer and the Honorary Fellow film are on **`eleven_v3`**, which takes emotional direction inline from the text | Narration in every master of all four films; `../explainer/audio/voice-test/` holds the A/B | ✅ **Delivered** | **~34,000 credits (~$6.20) in total, transcription and the music bed (3,749 credits) included.** Every segment generated one take each (`generations_count: 1`). **No voice cloning** — stock library voices, not clones, and no narrator presented as a named person. Limitations recorded: a bare `creative_list_voices` returned only 3 voices and the first narrator was picked from those 3 — searching properly returns 22, which the A/B corrected; `eleven_multilingual_v2` has no emotional direction, which is why the later films use `eleven_v3`; and the Honorary Fellow brief's script ran 146 s against its own 120 s cap at the pace it asked for, which cost one round of copy tightening and six retakes rather than a 16% speed-up of a narrator directed to be deliberate; a later benefits retake came back nine seconds long and was retaken tighter, and the pitch cut re-recorded nine scenes. |
