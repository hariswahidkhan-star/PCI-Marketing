...
| **ElevenLabs** | Surveyed voices, then generated **55 narration segments** — 15 for the 75-second film, 4 for the 15-second launch film, 16 for the 4:16 explainer, a 4-voice A/B, and 27 for the Honorary Fellow film (10 scenes, 6 tightened retakes, 2 benefits retakes and 9 for the pitch cut) | Three films on **"Jim Executive — Authoritative, British and Warm"** (`tXxkePQsw0G69D8VeDzp`). The Honorary Fellow film on **"Holden Pro Voice"** (`UudLhsL2DlHkDK0vGwl3`), the voice its brief names. The 75s and 15s films are on `eleven_multilingual_v2`; the explainer and the Honorary Fellow film are on **`eleven_v3`**, which takes emotional direction inline from the text | Narration in every master of all four films; `../explainer/audio/voice-test/` holds the A/B | ✅ **Delivered** | **~10,600 credits (~$1.95) in total.** Every segment generated one take each (`generations_count: 1`). **No voice cloning** — stock library voices, not clones, and no narrator presented as a named person. Limitations recorded: a bare `creative_list_voices` returned only 3 voices and the first narrator was picked from those 3 — searching properly returns 22, which the A/B corrected; `eleven_multilingual_v2` has no emotional direction, which is why the later films use `eleven_v3`; and the Honorary Fellow brief's script ran 146 s against its own 120 s cap at the pace it asked for, which cost one round of copy tightening and six retakes rather than a 16% speed-up of a narrator directed to be deliberate; a later benefits retake came back nine seconds long and was retaken tighter, and the pitch cut re-recorded nine scenes. |
