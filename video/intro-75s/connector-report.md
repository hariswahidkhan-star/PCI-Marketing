...
| **ElevenLabs** | Surveyed voices, then generated **19 narration segments** — 15 for the 75-second film and 4 for the 15-second launch film | **Chosen: "Jim Executive — Authoritative, British and Warm"** (`tXxkePQsw0G69D8VeDzp`). A library voice, described for corporate narration and documentary — matching the brief's "warm, authoritative, internationally understandable" | `dist/…-voiceover.wav`, `…-mixed-soundtrack.wav`, and the narration in every master of **both** films | ✅ **Delivered** | **~1,402 credits (~$0.25).** 19 segments generated one take each (`generations_count: 1`, since the default of 4 would have quadrupled the spend). **No voice cloning** — a stock library voice, not a clone, and not presented as a named person. The read came back ~19% slower than the film's design pace, which forced the timeline conform documented in the README. |
