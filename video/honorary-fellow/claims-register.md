...
- **The narration is synthetic** — ElevenLabs *"Nassim — Corporate Narration"*
  (`repzAAjoKlgcT2oOAIWt`) on `eleven_v3`, the stock library voice PCI named,
  directed phrase by phrase with emotional cues (`src/v3-emotion.json`). **It is
  not a clone of any real person**, and no real person's voice was used. Every
  take was transcribed back and matched to the script word for word before use.
- **The music is generated, not licensed stock** — a 250 s instrumental from
  ElevenLabs Music v2 in PCI's own workspace (`music/README.md`), replacing the
  standard-library score at PCI's request for professional background music.
  The original score is still built and delivered as a stem.
